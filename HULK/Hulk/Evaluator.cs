...
class Evaluator
{
    ...
    public StringExp EvaluateStringOp(StringOperation s)
    {
        ...
        {
            errorList.Add($"SEMANTIC ERROR: Invali not string type value for a string operation.");
            return null;
        }
    }
    ...
    public Expression EvaluateFunction(FunctionCall function)
    {
        ...
        {
            errorList.Add($"SEMANTIC ERROR: The number of parameters give for {function.Name} is incorrect ({function.Name} gets {function.Arguments.Count} parameters.)");
            return null; 
        }
